...
<div class="main-sidebar sidebar-style-2">
  <aside id="sidebar-wrapper">
    <div class="sidebar-brand">
      <a> <img alt="image" src="{{ asset('assets/img/logo.png') }}" class="wao's logo" /> <span class="logo-name"></span>
      </a>
    </div>
    <ul class="sidebar-menu">
      ...
      <li class="dropdown {{ (request()->is('home')) ? 'active' : '' }} ">
        <a href="{{route('admin.home')}}" class="nav-link"><i data-feather="monitor"></i><span>Wao Dashboard</span></a>
      </li>
      ...
      <li class="dropdown {{ (request()->segment(2) == 'product') ? 'active' : '' }}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-brands fa-product-hunt"></i>
      ...
      <li class="dropdown {{ (request()->segment(2) == 'order') ? 'active' : '' }}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-solid fa-cart-shopping"></i>
          <span>Orders</span></a>
        <ul class="dropdown-menu">
        ...
        <ul class="dropdown-menu">
          <li><a class="nav-link text-nowrap" href="{{route('order.shipper.advice')}}">View Shipper Advice &nbsp;&nbsp; @if(Cache::get('shipper_advice_count') > 0)<span class="rounded bg-danger text-white px-1 text-center"> {{ Cache::get('shipper_advice_count') }}</span>@endif</a></li>
        </ul>
        <ul class="dropdown-menu">
      ...
      <li class="dropdown {{  request()->routeIs('inventory.seller.index') ? 'active' : ''}}">
        ...
        <ul class="dropdown-menu">
          ...
          <li><a class="nav-link" href="{{route('waoseller.balance.history')}}">Balance History</a></li>
          @endcan
        </ul>
      </li>
      @endcan
      ...
      @can('view_inventory')
      <!-- wao Inventory -->
      <li class="dropdown {{  request()->routeIs('inventory.index') || request()->routeIs('waoseller.order.index') || request()->routeIs('ordersProfit') ? 'active' : ''}}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-solid fa-gear"></i><span>General Inventory-Sale</span></a>
        <ul class="dropdown-menu">
          ...
          @endcan
          <li><a class="nav-link" href="{{route('ordersProfit')}}?profit_transaction_status=pending">Reseller Profit Orders</a></li>
        </ul>
      </li>
      @endcan
      ...
      <li class="dropdown">
        <a href="{{route('admin.reviews')}}" class="nav-link">
          ...
          <span>General Reviews @if(Cache::get('pending_reviews_count') > 0)<span class="rounded bg-danger text-white px-1"> {{ Cache::get('pending_reviews_count') }}</span> @endif</span>
        </a>
      </li>
      @endcan
      ...
      <li class="dropdown {{ (request()->segment(2) == 'setting') ? 'active' : '' }}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-solid fa-gear"></i><span>App Manage</span></a>
        <ul class="dropdown-menu">
      ...
      <li class="dropdown {{  request()->routeIs('waoseller.order.index') || request()->routeIs('waoseller.balance.history') || request()->routeIs('waoseller.order.create') ? 'active' : ''}}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-solid fa-cart-shopping"></i>
        ...
        <ul class="dropdown-menu">
          ...
          <li><a class="nav-link" href="{{route('waoseller.getWebOrders', ['is_partner' => 1, 'status' => 'PENDING'])}}">TeamMember-Orders</a></li>
            <li><a class="nav-link" href="{{ route('inventory.seller.index', ['is_partner' => 1]) }}">Profiles</a></li>
          @endif
          <li><a class="nav-link" href="{{route('waoseller.balance.history')}}">Balance History</a></li>
        </ul>
      </li>

      <li class="dropdown {{ request()->routeIs('waoseller.getUsers') || request()->routeIs('waoseller.editUser') || request()->routeIs('waoseller.getWebOrders') || request()->routeIs('waoseller.editOrder') ? 'active' : ''}}">
        <a href="#" class="menu-toggle nav-link has-dropdown"><i class="fa-solid fa-cart-shopping"></i>
