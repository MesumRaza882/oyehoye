            ...
            <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-12 mb-2">
                        <form method="GET" action="{{ route('waoseller.getWebOrders') }}" id="search-form">
                            ...
                            <div class="row justify-content-between">
                                ...
                                <!-- if reseller has app orders option then select options -->
                                @if (auth()->user()->type === 3)
                                    <!-- select orders for web all app -->
                                    <div class="col-lg-2 mb-2 col-6">
                                        <label class="pb-0">Select Orders For</label>
                                        <select name="type" class="form-control me-2">
                                            <option value="">All</option>
                                            <option value="1" @if (request()->get('type') == 1) selected @endif>App
                                                Orders</option>
                                ...
                                {{-- when partner come this page to get team order and click for filter --}}
                ...
                <!-- <div class="col-12">
                                        <form action="{{ route('orders.genrate_slip') }}" method="POST" id="OrderGenrateSlip">
                                            ...
                                            <button class="btn btn-primary btn-sm mb-2" id="genrate_slip_btn" type="button">Genrate Slip</button>
                                        </form>
                                    </div> -->
                            ...
                            <thead>
                                <tr>
                                    <th><input type="checkbox" value="{{ request()->get('main_checkbox') }}"
                                            name="main_checkbox" id="select_all" style="background-color: aquamarine">
                                    </th>
                                    <th>id</th>
                                    <th>Nmae</th>
                                    <th>City</th>
                                ...
                                @foreach ($orders as $rate)
                                    <tr class="{{ $rate->is_active_row == 1 ? 'highlight' : '' }}">
                                        <td><input
                                                ...
                                                class="checkbox" value="{{ $rate->id }}" name="cat_checkbox"></td>
                                        <td>
                                            {{-- when partner come this page to get team order and click for filter --}}
                                            @if (request()->query('is_partner') == 1)
                                                <a href="{{ route('waoseller.editOrder', ['id' => $rate->id, 'is_partner' => 1]) }}"
                                                    class="btn btn-outline-primary text-dark">{{ $rate->id }}</a>
                                            @else
                                                <a href="{{ route('waoseller.editOrder', $rate->id) }}"
                                                    class="btn btn-outline-primary text-dark">{{ $rate->id }}</a>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <span>
                                                @php
                                                    ...
                                                    $hoursDifference = $currentTime->diffInHours($orderTrackStartTime);
                                                @endphp
                                                ...
                                                @elseif(120 - $hoursDifference > 0 && $rate->courier_tracking_id)
                                                    <span class="fw-bold text-danger">{{ 120 - $hoursDifference }}</span>
                                        ...
                                        <td>
                                            ...
                                            <span class="p-1 py-2  bg-info badge"
                                                style="font-size:14px;">{{ $rate->type === 1 ? 'app' : 'website' }}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="https://wa.me/+92{{ $rate->userdetail->whatsapp }}?text=Hi%20Welcome%20to%20WAO"
                                                target="_blank" class="text-dark btn-outline-success btn">
                                        ...
                                        <td>
                                            {{ $rate->waoSellerDetail ? $rate->waoSellerDetail->email : '' }}
                                        </td>
                                        <td>
