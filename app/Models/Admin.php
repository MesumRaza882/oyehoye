...
class Admin extends Authenticatable
{
    ...
    public function scopeControlledByAdmin($query)
    {
        if (auth()->user()->role == 1) {
            // Get all admin IDs where role is 1
            $adminIds = self::where('role', 1)->pluck('id')->toArray();
            return $query->whereIn('controlled_by_admin', $adminIds);
        } else {
            return $query->where('controlled_by_admin', auth()->user()->id);
        }
    }
}
